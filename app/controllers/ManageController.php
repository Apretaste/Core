...
class ManageController extends Controller
{
	...
	/**
	 * Show the report for the survey
	 *
	 * @author kuma
	 * */
	...
	public function surveyResultsCSVAction()
	{
		...
		foreach ($results as $field => $result)
		{
			...
			foreach ($result['pivots'] as $pivot => $label) $row[] = $label; 
			...
			foreach($result['results'] as $question)
			{
				...
				foreach($question['a'] as $ans)
				{
					...
					{
						if ( ! isset($ans['p'][$pivot])) $row[] = "--";
						else $row[] = number_format($ans['p'][$pivot] / $result['totals'][$ans['i']] * 100, 1);
					}
				}
				$csv[] = $row;
			}
		...
		{
			foreach ($row as $j => $cell) $csvtext .= '"'.$cell.'";';
			$csvtext .="\n";
		}
