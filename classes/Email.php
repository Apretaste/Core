...
class Email
{
	...
	/**
	 * Queue the email to be sent async.
	 * DO NOT use send() unless non-async is required.
	 *
	 * @author salvipascual
	...
	public function queue()
	{
		...
		$auth = Connection::query("INSERT INTO delivery_queue (data) VALUES ('$data')");

		$output = new stdClass();
		$output->code = "201";
		$output->message = "Email was queued";
		return $output;
	}
	...
	public function send()
	{
		...
		if (substr($this->to, -3) === ".cu") {
			$res = $this->sendEmailViaWebmail(); // try webmail
			if ($res->code != 200) $res = $this->sendEmailViaSSH(); // failover ssh
			//if($res->code != "200") $res = $this->sendEmailViaGmail(); // failover gmail
		}
		//
	...
	public function sendFromTemplate($template, $params = [], $layout = "email_empty.tpl")
	{
		...
		$this->body = Render::renderHTML(new Service(), $response);
	...
	public function sendEmailViaSSH()
	{
		...
		do {
			...
			//Set the Url
			curl_setopt($ch, CURLOPT_URL, 'http://10.0.0.4/web2smtp/');
